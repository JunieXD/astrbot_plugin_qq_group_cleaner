...
| 群内成员等级 | R/S/P | `level` 来自 `memberRealLevel`，缺失回退字符串 `"0"`；v0.2.2起按产品规则将接口返回的0作为有效群等级，字段本身缺失或非法仍保留 |
...
2026-09-21后续调查中，目标群803名群等级0成员的最后发言时间全部等于入群时间，1173名非0成员的最后发言时间全部晚于入群时间，抽查4名0级成员的详情仍返回0。v0.2.2据此及使用者要求，将接口群等级0直接按0级参与规则，不增加额外查询。此为产品处理约定；OneBot仍无法区分底层真实0与缺失回退，不代表证明了每名成员从未发言。以上早期抽样结论仅描述当时的接口边界，QQ等级的补查策略保持独立。
